...
export function isValidAddress(address: string): boolean {
  // Stellar addresses: G (public key) or C (contract), 56 chars total
 ...
 * @returns Validation result
 */
export function validateNFTConfig(config: {
  nft_type: 'Enumerable' | 'Royalties' | 'AccessControl';
  owner: string;
  admin?: string;
  manager?: string;
  salt: string;
  ...
  // Validate owner
  if (!isValidAddress(config.owner)) {
    errors.push(`Invalid owner address: ${config.owner}`);
  }

  // Type-specific validation
  if (config.nft_type === 'Royalties' || config.nft_type === 'AccessControl') {
    if (!config.admin) {
      errors.push(`${config.nft_type} NFT requires admin parameter`);
    } else if (!isValidAddress(config.admin)) {
      errors.push(`Invalid admin address: ${config.admin}`);
    }
  }

  if (config.nft_type === 'Royalties') {
    if (!config.manager) {
      errors.push('Royalties NFT requires manager parameter');
    } else if (!isValidAddress(config.manager)) {
      errors.push(`Invalid manager address: ${config.manager}`);
    }
  }
